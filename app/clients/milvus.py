...
def connect_milvus() -> bool:
    ...
    connections.connect(
        alias=_alias(),
        host=s.milvus_host, port=str(s.milvus_port),
        user=s.milvus_user or None, password=s.milvus_password or None,
    )
...
def search_vectors(collection: str, query_vec: list[float], top_k: int = 20,
