.PHONY: help install dev test lint fmt seed demo pull-llm build up down logs ps clean
...
help:
	...
	@echo "  make pull-llm   拉取 Ollama LLM (首次启动用)"
	@echo "  make down       docker compose 停服"
	@echo "  make logs       看 api 日志"
	@echo "  make seed       初始化 DB + 医学 KB + Demo 报告"
...
up:
	...
	@echo "== 首次使用还需要: make pull-llm 拉取大模型 =="
