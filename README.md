...
| 智能体 / Agent       | 能力 / Capability                                                                                    | 状态 |
| -------------------- | ---------------------------------------------------------------------------------------------------- | ---- |
| **报告解析 Agent**   | 图片 / PDF 双格式输入 · PaddleOCR + 版式识别 · 6 字段抽取 · 指标归一化 + 同义词映射 · 结构化 JSON 归档 | ✅ 简历内 |
| **医学解读 Agent**   | 医学知识库 RAG · 多指标联合分析 · ReAct 推理 · 四级风险分级 · 就医分级 · 危急值兜底                  | ✅ 简历内 |
| **健康管家 Agent**   | LangGraph 多 Agent 编排 · 多轮记忆 · 用户健康档案 · APScheduler 主动复查提醒 · FAQ 兜底               | 🚧 2025-10 ([Roadmap](#-beyond-resume--离职后持续打磨)) |
...
## 🚀 Beyond Resume · 离职后持续打磨

> 简历项目 2 描述的是 **2024.08 - 2025.03** 离职那刻的真实交付：2 个 Agent（报告解析 + 医学解读），覆盖"上传 → OCR → 解读 → 风险分级"线性工序。
> 本 repo 在简历基础上**继续演进**，按公开技术栈 mock 一条 Phase 2 路线，主要目的是把"单次解读型"产品延伸为"长期健康陪伴型"产品，
> 展示 *long-running agent / proactive scheduling / multi-agent orchestration* 这类 2026 年面试场景里高频被问的能力。

### Roadmap

| 月份 | 里程碑 | Release | 与简历的关系 |
|---|---|---|---|
| 2025-09 | **Web 上传页** · Vue3 + Vite 拖拽上传 / PDF & 图片预览 / 上传进度 / 解读结果展示 | `v1.1.0` | 补齐前端入口（简历内不展开） |
| 2025-10 | **健康管家 Agent**（headline · 简历外延）· LangGraph 多 Agent 编排 + 多轮记忆 + 主动复查提醒 | `v1.2.0` | 简历外的差异化能力 |
| 2025-11 | **历史报告趋势对比** · 同用户多次报告的指标趋势图 + 异常变化高亮 | `v1.3.0` | 健康管家 Agent 的支撑工具 |
| 2025-12 | **多模态影像解读** · CT / MRI / 超声报告 | `v1.4.0` | 简历外的新模态拓展 |

### 健康管家 Agent · 设计概览（2025-10 实现）

不替代「医学解读 Agent」，而是**把它当工具调用**——形成 *原始数据 → 单次解读 → 长期陪伴* 的三段式闭环：
